...
class Model:
    ...
    def __init__(self, input, dyn_batch_size, useSecondary=True, ngf=64, generatorOutputChannels=64, reuse_bool=False,
                 pooling_type="max", last_convolutions_channels=[64, 32, 9], useCoordConv=False, firstAsGuide=False,
                 NoMaxPooling=False):
            ...
            with tf.variable_scope("globalNetwork_fc_%d" % (layerCount + 1)):
                ...
                if not lastGlobalNetworkValue is None:
                    nextGlobalInput = tf.concat(
                        [tf.expand_dims(tf.expand_dims(lastGlobalNetworkValue, axis=1), axis=1), inputMean], axis=-1)
                globalNetwork_fc = tfHelpers.fullyConnected(nextGlobalInput, nextChannels, True,
                                                            "globalNetworkLayer" + str(layerCount + 1))

            return summed, tf.nn.selu(
                globalNetwork_fc)  # returns the sum of this layer + last globalNet output and a new globalNetValue
        else:
            return input, None
    ...
    def __create_encoder(self, input):
        ...
        for layerCount, out_channels in enumerate(layer_specs):
            with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
                ...
                outputs, lastGlobalNet = self._addSecondaryNetBlock(outputs, mean, lastGlobalNet, out_channels,
                                                                    layer_specs[layers_specs_GlobalNet],
                                                                    len(layers) + 1)
        ...
        with tf.variable_scope("encoder_8"):
            ...
            convolved, lastGlobalNet = self._addSecondaryNetBlock(convolved, None, lastGlobalNet, self.ngf * 8,
                                                                  self.ngf * 8, len(layers) + 1, keep_dims=True)
    ...
    def __create_decoder(self, encoder_results, lastGlobalNet, output_channels):
        layer_specs = [
            ...
            (self.ngf * 8, 0.5),
            # decoder_6: [batch, 4, 4, ngf * 8 ] => [batch, 8, 8, ngf * 8] #Dropout was 0.5 until here
        ...
        for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
            ...
            with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
                ...
                output, lastGlobalNet = self._addSecondaryNetBlock(output, mean, lastGlobalNet, out_channels,
                                                                   out_channels,
                                                                   num_encoder_layers + len(decoder_results))
                if dropout > 0.0:
        ...
        with tf.variable_scope("decoder_1"):
            ...
            deconved, lastGlobalNet = self._addSecondaryNetBlock(deconved, None, lastGlobalNet, output_channels,
                                                                 output_channels,
                                                                 num_encoder_layers + len(decoder_results), True)
    ...
    def __createLastConvs(self, input, secondaryNet_input, output_channels, last_channels, reuse_bool=True):
        input, lastGlobalNet = self._addSecondaryNetBlock(input, None, secondaryNet_input, last_channels,
                                                          output_channels[0], 0, True)
        # if self.useCoordConv:
        #    coords = helpers.generateCoords(tf.shape(input))
        #    input = tf.concat([input, coords], axis = -1)
        layers = [input]
        for layerCount, chanCount in enumerate(output_channels[:-1]):
            with tf.variable_scope("final_conv_" + str(layerCount)):
                convolved = tfHelpers.conv(layers[-1], chanCount, stride=1, filterSize=3, initScale=0.02,
                                           useXavier=False, paddingSize=1)
                lastLayerResult, mean, variance = tfHelpers.instancenorm(convolved)

                lastLayerResult, lastGlobalNet = self._addSecondaryNetBlock(lastLayerResult, mean, lastGlobalNet,
                                                                            chanCount, output_channels[layerCount + 1],
                                                                            len(layers))
                rectified = tfHelpers.lrelu(lastLayerResult, 0.2)
                layers.append(rectified)
        with tf.variable_scope("final_conv_last"):
            convolved = tfHelpers.conv(layers[-1], output_channels[-1], stride=1, filterSize=3, initScale=0.02,
                                       useXavier=True, paddingSize=1, useBias=True)
    ...
    def create_generator(self, generator_inputs, generator_outputs_channels, reuse_bool=True):
        with tf.variable_scope("generator", reuse=reuse_bool) as scope:
            ...
            with tf.variable_scope("globalNetwork_fc_1"):
                globalNetwork_fc_1 = tfHelpers.fullyConnected(globalNetworkInput, self.ngf * 2, True,
            ...
            for layerCount, out_channels in enumerate(layer_specs):
                with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
                    ...
                    with tf.variable_scope("globalNetwork_fc_%d" % (len(globalNetworkOutputs) + 1)):
                        ...
                        globalNetwork_fc = ""
                        if layerCount + 1 < len(layer_specs) - 1:
                            globalNetwork_fc = tfHelpers.fullyConnected(nextGlobalInput, layer_specs[layerCount + 1],
                                                                        True, "globalNetworkLayer" + str(
                                    len(globalNetworkOutputs) + 1))
                        else:
                            globalNetwork_fc = tfHelpers.fullyConnected(nextGlobalInput, layer_specs[layerCount], True,
                                                                        "globalNetworkLayer" + str(
                                                                            len(globalNetworkOutputs) + 1))
            ...
            with tf.variable_scope("encoder_8"):
                ...
                with tf.variable_scope("globalNetwork_fc_%d" % (len(globalNetworkOutputs) + 1)):
                    ...
                    nextGlobalInput = tf.concat(
                        [tf.expand_dims(tf.expand_dims(globalNetworkOutputs[-1], axis=1), axis=1), mean], axis=-1)
                    globalNetwork_fc = tfHelpers.fullyConnected(nextGlobalInput, self.ngf * 8, True,
                                                                "globalNetworkLayer" + str(
                                                                    len(globalNetworkOutputs) + 1))
                    globalNetworkOutputs.append(tf.nn.selu(globalNetwork_fc))
            ...
            for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
                ...
                with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
                    ...
                    with tf.variable_scope("globalNetwork_fc_%d" % (len(globalNetworkOutputs) + 1)):
                        nextGlobalInput = tf.concat(
                            [tf.expand_dims(tf.expand_dims(globalNetworkOutputs[-1], axis=1), axis=1), mean], axis=-1)
                        globalNetwork_fc = tfHelpers.fullyConnected(nextGlobalInput, out_channels, True,
                                                                    "globalNetworkLayer" + str(
                                                                        len(globalNetworkOutputs) + 1))
                        globalNetworkOutputs.append(tf.nn.selu(globalNetwork_fc))
                    if dropout > 0.0:
    ...
    def create_model(self):
        with tf.variable_scope("trainableModel", reuse=self.reuse_bool) as scope:
            ...
            generator_output, secondary_output = self.__create_generator(self.inputTensor, self.generatorOutputChannels,
                                                                         self.reuse_bool)
            ...
            if not self.NoMaxPooling:
                ...
                tmpOutputs = tf.reshape(pooledGeneratorOutput,
                                        [self.dynamic_batch_size, -1, tf.shape(generator_output)[1],
                                         tf.shape(generator_output)[2], int(generator_output.get_shape()[3])])
                tmpSecondary = tf.reshape(pooledSecondaryOutput,
                                          [self.dynamic_batch_size, -1, int(secondary_output.get_shape()[1])])
                ...
                if self.firstAsGuide:
                    ...
                    pooledGeneratorOutput = tf.concat([tmpOutputs[:, 0], pooledGeneratorOutput],
                                                      axis=-1)  # should now have twice the nb of channels

                    pooledSecondaryOutput = tfHelpers.pooling(self.pooling_type, tmpSecondary, self.dynamic_batch_size)
                    pooledSecondaryOutput = tf.concat([tmpSecondary[:, 0], pooledSecondaryOutput],
                                                      axis=-1)  # should now have twice the nb of channels
            ...
            partialOutput = self.__createLastConvs(pooledGeneratorOutput, pooledSecondaryOutput,
                                                   self.last_convolutions_channels, self.generatorOutputChannels)
