...
class GGXRenderer:
    ...
    def tf_Render(self, svbrdf, wi, wo, currentConeTargetPos, tensorboard="", multiLight=False, currentLightPos=None,
                  lossRendering=True, isAmbient=False, useAugmentation=True):
        ...
        if not lossRendering:
            ...
            if not isAmbient:
                if useAugmentation:
                    ...
                    lampIntensity = tf.abs(tf.random_normal((resultShape[0], resultShape[1], 1, 1, 1), mean=10.0,
                                                            stddev=stdDevWholeBatch))  # Creates a different lighting condition for each shot of the nbRenderings Check for over exposure in renderings
                    # autoExposure
                    autoExposure = tf.exp(tf.random_normal((), mean=np.log(1.5), stddev=0.4))
                    lampIntensity = lampIntensity * autoExposure
                else:
                    lampIntensity = tf.reshape(tf.constant(13.0),
                                               [1, 1, 1, 1, 1])  # Look at the renderings when not using augmentations
            else:
                # If this uses ambient lighting we use much small light values to not burn everything.
                if useAugmentation:
                    lampIntensity = tf.exp(tf.random_normal((resultShape[0], 1, 1, 1, 1), mean=tf.log(0.15),
                                                            stddev=0.5))  # No need to make it change for each rendering.
                else:
            ...
            if useAugmentation and not isAmbient:
                whiteBalance = tf.abs(
                    tf.random_normal([resultShape[0], resultShape[1], 1, 1, 3], mean=1.0, stddev=0.03))
        ...
        if multiLight:
            result = tf.reduce_sum(result,
                                   axis=2) * 1.0  # if we have multiple light we need to multiply this by (1/number of lights).
        if lossRendering:
            result = result / tf.expand_dims(tf.maximum(wiNorm[:, :, :, :, 2], 0.001),
