...
class Loss:
    ...
    def __l2Loss(self):
        ...
        return l2(outputsNormal, targetsNormal) + l2(outputsDiffuse, targetsDiffuse) + l2(outputsRoughness,
                                                                                          targetsRoughness) + l2(
            outputsSpecular, targetsSpecular)
    ...
    def __l1Loss(self):
        ...
        return l1(outputsNormal, targetsNormal) + l1(outputsDiffuse, targetsDiffuse) + l1(outputsRoughness,
                                                                                          targetsRoughness) + l1(
            outputsSpecular, targetsSpecular)
    ...
    def __generateRenderings(self, renderer):
        diffuses = helpers.tf_generateDiffuseRendering(self.batchSize, self.nbDiffuseRendering, self.targets,
                                                       self.outputs, renderer)
        speculars = helpers.tf_generateSpecularRendering(self.batchSize, self.nbSpecularRendering, self.surfaceArray,
                                                         self.targets, self.outputs, renderer)
    ...
    def __renderLoss(self):
        ...
        targetsLogged = tf.log(
            self.targetsRenderings + epsilonRender)  # Bias could be improved to 0.1 if the network gets upset.
        outputsLogged = tf.log(self.outputsRenderings + epsilonRender)
        lossTotal = l1(targetsLogged,
                       outputsLogged)  # 0.5 * l1(targetsLogged, outputsLogged) + l1(DX(targetsLogged), DX(outputsLogged)) + l1(DY(targetsLogged), DY(outputsLogged))
