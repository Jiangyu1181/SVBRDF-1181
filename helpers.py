...
def tf_generate_distance(batchSize, nbRenderings):
    gaussian = tf.random_normal([batchSize, nbRenderings, 1], 0.5, 0.75,
...
def adaptAlbedos(mixedMaterial, batchSize):
    ...
    multiplier = tf.concat(
        [tf.ones([batchSize, 1]), tf.expand_dims(randomAlbedoMultipliers[:, 0], axis=-1), tf.ones([batchSize, 1]),
         tf.expand_dims(randomAlbedoMultipliers[:, 1], axis=-1)], axis=-1)
...
def adaptNormals(mixedMaterial, batchSize):
    ...
    boolValue = tf.floor(tf.minimum(shouldRemoveNormal,
                                    1.0))  # 0 if shouldRemoveNormal>=0 && <= 1 ; 1 if > 1. So it has (100/5) % of chance to happen
    ...
    mixedMaterial = tf.concat([tf.expand_dims(normals, axis=1), tf.expand_dims(mixedMaterial[:, 1], axis=1),
                               tf.expand_dims(roughness, axis=1), tf.expand_dims(mixedMaterial[:, 3], axis=1)], axis=1)
...
def cutSidesOut(inputs, targetstoRender, topLeft, cropSize, firstAsGuide, averageCrop):
    ...
    croppedInputs = tf.reshape(croppedInputs,
                               [inputOriginalShape[0], inputOriginalShape[1], newInputShape[1], newInputShape[2],
                                newInputShape[3]])
...
def mixMaterials(material1, material2, alpha):
    ...
    normal1Projected = normal1Corrected / tf.expand_dims(tf.maximum(0.01, normal1Corrected[:, :, :, 2]),
                                                         axis=-1)  # Project the normals to use the X and Y derivative
...
def generateCoords(inputShape):
    ...
    coords = tf.expand_dims(coords,
                            axis=0)  # Add dimension to support batch size and nbRenderings should now be [1, 256, 256, 2].
...
def generateSurfaceArray(crop_size, pixelsToAdd=0):
    ...
    surfaceArray = tf.concat([XsurfaceArray, YsurfaceArray, tf.zeros([totalSize, totalSize, 1], dtype=tf.float32)],
                             axis=-1)
    surfaceArray = tf.expand_dims(tf.expand_dims(surfaceArray, axis=0),
...
def jitterPosAround(batchSize, nbRenderings, posTensor, mean=0.0, stddev=0.03):
    randomPerturbation = tf.clip_by_value(
        tf.random_normal([batchSize, nbRenderings, 1, 1, 1, 3], mean, stddev, dtype=tf.float32), -0.24,
        0.24)  # Clip here how far it can go to 8 * stddev to avoid negative values on view or light ( Z minimum value is 0.3)
...
def generateInputRenderings(rendererInstance, material, batchSize, nbRenderings, surfaceArray, renderingScene,
                            jitterLightPos, jitterViewPos, useAmbientLight, useAugmentationInRenderings=True):
    currentLightPos, currentViewPos, currentConeTargetPos = None, None, None
    if renderingScene == "staticViewPlaneLight":
        currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray, useAugmentation = acquisitionScene.defaultScene(
            surfaceArray, batchSize, nbRenderings)
    elif renderingScene == "staticViewSpotLight":
        currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray, useAugmentation = acquisitionScene.defaultSceneSpotLight(
            surfaceArray, batchSize, nbRenderings)
    elif renderingScene == "staticViewHemiSpotLight":
        currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray, useAugmentation = acquisitionScene.fixedViewHemisphereConeLight(
            surfaceArray, batchSize, nbRenderings)
    elif renderingScene == "staticViewHemiSpotLightOneSurface":
        currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray, useAugmentation = acquisitionScene.fixedViewHemisphereConeLightOneOnPlane(
            surfaceArray, batchSize, nbRenderings)
    elif renderingScene == "movingViewHemiSpotLightOneSurface":
        currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray, useAugmentation = acquisitionScene.movingViewHemisphereConeLightOneOnPlane(
            surfaceArray, batchSize, nbRenderings, useAugmentationInRenderings)
    elif renderingScene == "fixedAngles":
        currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray, useAugmentation = acquisitionScene.fixedAngles(
            surfaceArray, batchSize, nbRenderings, useAugmentationInRenderings)
        ...
        currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray, useAugmentation = acquisitionScene.globalTestScene(
            surfaceArray, batchSize, nbRenderings, useAugmentationInRenderings)
    else:
    ...
    if useAmbientLight:
        ...
        renderingAmbient = \
        rendererInstance.tf_Render(material, wiAmbient, wo, None, multiLight=True, currentLightPos=ambientLightPos,
                                   lossRendering=False, isAmbient=True, useAugmentation=useAugmentation)[0]
    wi = currentLightPos - surfaceArray

    # tf.summary.image("wi", convert(deprocess(wi[:, 0,0])), max_outputs=5)

    renderings = \
    rendererInstance.tf_Render(material, wi, wo, currentConeTargetPos, multiLight=True, currentLightPos=currentLightPos,
                               lossRendering=False, isAmbient=False, useAugmentation=useAugmentation)[
        0]  # was currentConeTargetPos not None
    if useAmbientLight:
    ...
    renderings = tf.clip_by_value(renderings, 0.0,
                                  1.0)  # Make sure noise doesn't put values below 0 and simulate over exposure
...
def tf_generateDiffuseRendering(batchSize, nbRenderings, targets, outputs, renderer):
    ...
    renderedDiffuseOutputs = renderer.tf_Render(outputs, wi, wo, None, "", useAugmentation=False, lossRendering=True)[
        0]  # tf_Render_Optis(outputs,wi,wo)
...
def tf_generateSpecularRendering(batchSize, nbRenderings, surfaceArray, targets, outputs, renderer):
    ...
    currentShift = tf.concat([tf.random_uniform([batchSize, nbRenderings, 2], -1.0, 1.0),
                              tf.zeros([batchSize, nbRenderings, 1], dtype=tf.float32) + 0.0001], axis=-1)
    ...
    renderedSpecularOutputs = renderer.tf_Render(outputs, wi, wo, None, "", useAugmentation=False, lossRendering=True)[
        0]
...
def input_reshape(inputs, NoMaxPooling, nbRenderings):
    ...
    if NoMaxPooling:
        ...
        reshaped = tf.reshape(inputs, [batchSize * nbInputs, int(inputs.get_shape()[2]), int(inputs.get_shape()[3]),
                                       int(inputs.get_shape()[4])])  # Should be [batchSize * nbInputs, 256,256,3]
...
def deprocess_outputs(outputs):
    partialOutputedNormals = outputs[:, :, :,
                             0:2] * 3.0  # The multiplication here gives space to generate direction with angle > pi/4
...
def registerTensorboard(paths, images, nbInputsMax, nbTargets, loss_value, batch_size, targetsRenderings,
                        outputsRenderings):
    inputs = images[0]
    targets = images[1]
    outputs = images[2]
...
def deprocess_images(inputs, targets, outputs, gammaCorrectedInputs, nbTargets, logAlbedo):
    ...
    with tf.name_scope("transform_images"):
        ...
        targets_reshaped = reshape_tensor_display(targets, nbTargets, logAlbedo,
                                                  axisToSplit=1)  # tf.reshape(targets, [-1, int(targetShape[2]), int(targetShape[3]), int(targetShape[4])])
        ...
        gammaCorrectedInputs_reshaped = tf.reshape(gammaCorrectedInputs, [-1, tf.shape(gammaCorrectedInputs)[2],
                                                                          tf.shape(gammaCorrectedInputs)[3],
                                                                          tf.shape(gammaCorrectedInputs)[4]])
...
def display_images_fetches(paths, inputs, targets, gammaCorrectedInputs, outputs, nbTargets, logAlbedo):
    converted_inputs, converted_targets, converted_outputs, converted_gammaCorrectedInputs = deprocess_images(inputs,
                                                                                                              targets,
                                                                                                              outputs,
    ...
    with tf.name_scope("encode_images"):
        display_fetches = {
            ...
            "gammaCorrectedInputs": tf.map_fn(tf.image.encode_png, converted_gammaCorrectedInputs, dtype=tf.string,
...
def save_images(fetches, output_dir, batch_size, nbTargets, step=None):
    ...
    for i, in_path in enumerate(fetches["paths"]):
        ...
        nbCurrentInput = len(
            fetches["inputs"]) // batch_size  # This only works if the nb of rendering is constant in the batch.
