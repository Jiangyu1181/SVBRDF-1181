...
def deprocess_images_fullTest(inputs, targets, nbTargets):
    ...
    with tf.name_scope("transform_images"):
        ...
        tensorOneInput, SurfaceLightFixedView, HemishpereLightFixedView = concatSplitInputs(inputs, axisToConcat=2,
...
def display_images_fetches_fullTest(paths, inputs, targets, nbTargets):
    tensorOneInput, SurfaceLightFixedView, HemishpereLightFixedView = deprocess_images_fullTest(inputs, targets,
                                                                                                nbTargets)  # , HemishpereLightHemisphereView
    with tf.name_scope("encode_images"):
        display_fetches = {
            "paths": paths,
            "tensorOneInput": tf.map_fn(tf.image.encode_png, tensorOneInput, dtype=tf.string,
                                        name="tensorOneInput_pngs"),
            "SurfaceLightFixedView": tf.map_fn(tf.image.encode_png, SurfaceLightFixedView, dtype=tf.string,
                                               name="SurfaceLightFixedView_pngs"),
            "HemishpereLightFixedView": tf.map_fn(tf.image.encode_png, HemishpereLightFixedView, dtype=tf.string,
...
def deprocess_images(inputs, targets, nbTargets):
    ...
    with tf.name_scope("transform_images"):
        targetShape = targets.get_shape()
        targets_reshaped = concat_tensor_display(targets, axisToConcat=2, axisToSplit=1)
        inputs_reshaped = tf.reshape(inputs, [-1, int(inputs.get_shape()[2]), int(inputs.get_shape()[3]),
...
def save_images_fullPath(fetches, output_dir, batch_size, nbTargets, step):
    folders = ["tensorOneInput", "SurfaceLightFixedView",
               "HemishpereLightFixedView"]  # , "HemishpereLightHemisphereView"]
...
def renderTests(input_dir, testFolder, maxInputNb, tmpFolder, imageFormat, CROP_SIZE, nbTargets, input_size, batchSize,
                renderingScene, jitterLightPos, jitterViewPos, inputMode, mode, outputDir):
    ...
    data = dataReader.dataset(input_dir, imageType=imageFormat, testFolder=testFolder, maxInputToRead=maxInputNb,
                              nbTargetsToRead=nbTargets, cropSize=CROP_SIZE, inputImageSize=input_size,
                              batchSize=batchSize, fixCrop=True, mixMaterials=True, fixImageNb=True, logInput=False,
                              useAmbientLight=False, jitterRenderings=False, useAugmentationInRenderings=False)
    ...
    display_fetches = display_images_fetches_fullTest(data.pathBatch, data.gammaCorrectedInputsBatch, data.targetBatch,
                                                      nbTargets)  # display_images_fetches(data.pathBatch, data.gammaCorrectedInputsBatch, data.targetBatch, nbTargets) # save the gamma corrected version of the inputs
    ...
    with sv.managed_session() as sess:
        ...
        for step in range(max_steps):
            try:
                ...
                save_images_fullPath(results, fullTmpFolder, batchSize, nbTargets,
                                     step)  # save_images(results, fullTmpFolder, batchSize, nbTargets)
