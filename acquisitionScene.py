...
def defaultSceneSpotLight(surfaceArray, batchSize, nbRenderings):
    currentLightPos, currentViewPos, currentConeTargetPos, surfaceArray = defaultScene(surfaceArray, batchSize,
                                                                                       nbRenderings)
...
def testSetLight(batchSize, nbRenderings):
    ...
    currentLightPos = tf.expand_dims(tf.concat([currentLightPos, tf.ones([batchSize, 1, 1]) * lightDistance], axis=-1),
                                     axis=-2)
...
def testSetConeTarget(batchSize, nbRenderings):
    nbRenderingEach = int((nbRenderings - 1) / 2)
    currentConeTargetPos0 = tf.random_normal([batchSize, 1, 1, 1, 1, 2], 0.0, 0.01,
                                             dtype=tf.float32)  # Give a first pos to 0
    ...
    currentConeTargetPos2 = tf.concat([currentConeTargetPos2, tf.zeros([batchSize, nbRenderingEach, 1, 1, 1, 1])],
                                      axis=-1)
...
def fullRandomLightsHemisphere(batchSize, nbRenderings):
    currentLightPos = tf.expand_dims(
        helpers.tf_generate_normalized_random_direction(batchSize, nbRenderings, lowEps=minEps,
                                                        highEps=maxEps) * lightDistance,
        axis=-2)  # getting the pos and adding the multi light dim.
...
def randomLightsHemisphereOneSurface(batchSize, nbRenderings):
    ...
    currentLightPos2 = tf.expand_dims(
        helpers.tf_generate_normalized_random_direction(batchSize, nbRenderings - 1, lowEps=minEps,
                                                        highEps=maxEps) * lightDistance,
        axis=-2)  # getting the pos and adding the multi light dim.
...
def randomViewHemisphereOneSurface(batchSize, nbRenderings, useAugmentation):
    # viewDistanceLocal = viewDistance
    if useAugmentation:
        viewDistanceLocal = tf.random_uniform((batchSize, nbRenderings, 1), 2.75,
                                              0.25)  # Simulates a FOV between 30 degrees and 50 degrees centered around 40 degrees
    else:
        viewDistanceLocal = tf.ones([batchSize, nbRenderings,
                                     1]) * viewDistance  # tf.tile(tf.reshape(tf.constant(viewDistance),[1,1,1]), [batchSize, nbRenderings, 1])
    currentViewPos = tf.random_uniform([batchSize, 1, 2], -0.25, 0.25, dtype=tf.float32)
    currentViewPos = tf.concat(
        [currentViewPos, tf.ones([batchSize, 1, 1]) * tf.expand_dims(viewDistanceLocal[:, 0, :], axis=1)], axis=-1)

    currentViewPos2 = helpers.tf_generate_normalized_random_direction(batchSize, nbRenderings - 1, lowEps=minEps,
                                                                      highEps=maxEps) * viewDistanceLocal[:, 1:,
                                                                                        :]  # tf.constant([0.0, 0.0, viewDistanceLocal])
...
def fullRandomLightsSurface(batchSize, nbRenderings):
    ...
    currentLightPos = tf.expand_dims(
        tf.concat([currentLightPos, tf.ones([batchSize, nbRenderings, 1]) * lightDistance], axis=-1), axis=-2)
...
def generateAmbientLight(currentLightPos,
                         batchSize):  # Maybe use the same cos distribution as in the other direction and pick a distance from a exp normal distribution
    ...
    ambiantDir = helpers.tf_generate_normalized_random_direction(batchSize, 1, lowEps=minEps,
                                                                 highEps=0.2)  # Here remove angles below 25 ° from the surface
    ambiantDir = tf.expand_dims(ambiantDir, axis=-2)
    ambiantDir = tf.expand_dims(ambiantDir, axis=-2)
    ambiantDir = tf.expand_dims(ambiantDir, axis=-2)

    ambiantPos = ambiantDir * tf.exp(tf.random_normal((), np.log(30.0), 0.15,
                                                      tf.float32))  # sample between 1.6 m to 5.4m if the sample is 20cm square
