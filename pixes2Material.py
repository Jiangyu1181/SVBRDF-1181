...
parser.add_argument("--loss", type=str, default="render", choices=["l1", "render", "mixed"],
...
parser.add_argument("--poolingtype", type=str, default="max", choices=["max", "mean"],
                    help="Define the type of pooling to use")
...
parser.add_argument("--inputMode", type=str, default="auto", choices=["auto", "xml", "folder", "image"],
                    help="What kind of input to expect")
...
parser.add_argument("--maxImages", type=int, default=1,
                    help="Maximum number of images for the full test, will run the test with 1 to maxImages")
parser.add_argument("--fixImageNb", dest="fixImageNb", action="store_true",
                    help="Use a fix number of image for the training.")
parser.set_defaults(fixImageNb=False)

parser.add_argument("--feedMethod", type=str, default="render", choices=["files", "render"],
                    help="Which feeding method to use")
parser.add_argument("--renderingScene", type=str, default="staticViewPlaneLight",
                    choices=["staticViewPlaneLight", "staticViewSpotLight", "staticViewHemiSpotLight",
                             "staticViewHemiSpotLightOneSurface", "movingViewHemiSpotLightOneSurface", "fixedAngles",
                             "globalTestScene"], help="Static view with plane light")
...
parser.add_argument("--useCoordConv", dest="useCoordConv", action="store_true",
                    help="use coordconv in the first convolution slot.")
parser.set_defaults(useCoordConv=False)
parser.add_argument("--useAmbientLight", dest="useAmbientLight", action="store_true",
                    help="use ambient lighting in the rendering.")
parser.set_defaults(useAmbientLight=False)
parser.add_argument("--jitterRenderings", dest="jitterRenderings", action="store_true",
                    help="spatially jitter the renderings.")
parser.set_defaults(jitterRenderings=False)
parser.add_argument("--firstAsGuide", dest="firstAsGuide", action="store_true",
                    help="Use the first picture provided as a guide.")
...
parser.add_argument("--NoAugmentationInRenderings", dest="NoAugmentationInRenderings", action="store_true",
                    help="Use the max pooling system.")
...
def main():
    ...
    if a.mode == "test" and a.feedMethod == "render":
        testHelpers.renderTests(a.input_dir, a.testFolder, a.maxImages, tmpFolder, a.imageFormat, CROP_SIZE,
                                a.nbTargets, a.input_size, a.batch_size, a.renderingScene, a.jitterLightPos,
                                a.jitterViewPos, a.inputMode, a.mode, a.output_dir)
    ...
    for runID in range(nbRun):
        ...
        data = dataReader.dataset(a.input_dir, imageFormat=a.imageFormat, trainFolder=a.trainFolder,
                                  testFolder=a.testFolder, inputNumbers=a.nbInputs, maxInputToRead=maxInputNb,
                                  nbTargetsToRead=a.nbTargets, cropSize=CROP_SIZE, inputImageSize=a.input_size,
                                  batchSize=a.batch_size, fixCrop=(a.mode == "test"), mixMaterials=(a.mode == "train"),
                                  fixImageNb=a.fixImageNb, logInput=a.useLog, useAmbientLight=a.useAmbientLight,
                                  jitterRenderings=a.jitterRenderings, firstAsGuide=False,
        ...
        if a.feedMethod == "render":
            data.populateInNetworkFeedGraph(a.renderingScene, a.jitterLightPos, a.jitterViewPos, a.mode == "test",
        ...
        if a.mode == "train":
            with tf.name_scope("recurrentTest"):
                ...
                dataTest = dataReader.dataset(a.input_dir, imageFormat=a.imageFormat, testFolder=a.testFolder,
                                              inputNumbers=a.nbInputs, maxInputToRead=a.maxImages,
                                              nbTargetsToRead=a.nbTargets, cropSize=CROP_SIZE,
                                              inputImageSize=a.input_size, batchSize=a.batch_size, fixCrop=True,
                                              mixMaterials=False, fixImageNb=a.fixImageNb, logInput=a.useLog,
                                              useAmbientLight=a.useAmbientLight, jitterRenderings=a.jitterRenderings,
                                              firstAsGuide=a.firstAsGuide,
                                              useAugmentationInRenderings=not a.NoAugmentationInRenderings, mode=a.mode)
                dataTest.loadPathList(a.inputMode, "test", False)
                if a.feedMethod == "render":
                    dataTest.populateInNetworkFeedGraph(a.renderingScene, a.jitterLightPos, a.jitterViewPos, True,
                                                        shuffle=False)
                elif a.feedMethod == "files":
                    dataTest.populateFeedGraph(False)
                TestinputReshaped, test_dyn_batch_size = helpers.input_reshape(dataTest.inputBatch, a.NoMaxPooling,
                                                                               a.maxImages)
        ...
        model = mod.Model(inputReshaped, dyn_batch_size, last_convolutions_channels=last_convs_chans,
                          generatorOutputChannels=64, useCoordConv=a.useCoordConv, firstAsGuide=a.firstAsGuide,
                          NoMaxPooling=a.NoMaxPooling, pooling_type=a.poolingtype)
        ...
        if a.mode == "train":
            ...
            testmodel = mod.Model(TestinputReshaped, test_dyn_batch_size, last_convolutions_channels=last_convs_chans,
                                  generatorOutputChannels=64, reuse_bool=True, useCoordConv=a.useCoordConv,
                                  firstAsGuide=a.firstAsGuide, NoMaxPooling=a.NoMaxPooling, pooling_type=a.poolingtype)
            testmodel.create_model()

            # Organize the images we want to retrieve from the test network run
            display_fetches_test, _ = helpers.display_images_fetches(dataTest.pathBatch, dataTest.inputBatch,
                                                                     dataTest.targetBatch,
                                                                     dataTest.gammaCorrectedInputsBatch,
                                                                     testmodel.output, a.nbTargets, a.logOutputAlbedos)

            # Compute the training network loss.
            loss = losses.Loss(a.loss, model.output, targetsReshaped, CROP_SIZE, a.batch_size,
                               tf.placeholder(tf.float64, shape=(), name="lr"), a.includeDiffuse)
        ...
        display_fetches, converted_images = helpers.display_images_fetches(data.pathBatch, data.inputBatch,
                                                                           data.targetBatch,
                                                                           data.gammaCorrectedInputsBatch, model.output,
                                                                           a.nbTargets, a.logOutputAlbedos)
        if a.mode == "train":
            # Register inputs, targets, renderings and loss in Tensorboard
            helpers.registerTensorboard(data.pathBatch, converted_images, a.maxImages, a.nbTargets, loss.lossValue,
                                        a.batch_size, loss.targetsRenderings, loss.outputsRenderings)
...
def loadCheckpointOption(mode=a.mode, checkpoint=a.checkpoint):
    if mode == "test" or mode == "eval" or mode == "testfull":
        ...
        options = {"which_direction", "ngf", "nbTargets", "loss", "useLog", "useCoordConv", "includeDiffuse",
                   "NoMaxPooling"}
...
def train(sv, sess, data, max_steps, display_fetches, display_fetches_test, dataTest, saver, loss,
        ...
        for step in range(max_steps):
            ...
            try:
                ...
                if a.checkpoint is None and step < 2000:
                    currentLrValue = step * (
                        0.0005) * a.lr  # ramps up to a.lr in the 2000 first iterations to avoid crazy first gradients to have too much impact.

                # Run the network
                results = sess.run(fetches, feed_dict={loss.lr: currentLrValue}, options=options,
            ...
            if helpers.should(a.progress_freq, max_steps, step):
                ...
                train_epoch = math.ceil(
                    global_step / data.stepsPerEpoch)  # global_step will have the correct step count even if we resume from a checkpoint
...
def runNetwork(inputDir, outputDir, checkpoint, maxImages, inputMode="image", feedMethod="files", mode="test",
               nbInputs=10, input_size=256, nbTargets=4):
