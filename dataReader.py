...
class dataset:
    ...
    def __init__(self, inputPath, trainFolder="train", testFolder="test", inputNumbers=10, maxInputToRead=1,
                 nbTargetsToRead=4, cropSize=256, inputImageSize=288, batchSize=1, imageFormat="png",
                 which_direction="AtoB", fixCrop=False, mixMaterials=True, fixImageNb=False, logInput=False,
                 useAmbientLight=False, jitterRenderings=False, firstAsGuide=False, useAugmentationInRenderings=True,
                 mode="train"):
    ...
    def __readImages(self, filename):
        ...
        with tf.control_dependencies([assertion]):
            ...
            floatInputSplit = tf.split(floatInput, self.nbTargetsToRead + self.inputNumbers, axis=1,
                                       name="Split_input_data")  # Splitted we get a list of nbTargets + inputNumbers images
    ...
    def __renderInputs(self, materials, renderingScene, jitterLightPos, jitterViewPos, mixMaterials, isTest,
                       renderSize):
        ...
        reshaped_targets_batch = helpers.target_reshape(
            mixedMaterial)  # reshape it to be compatible with the rendering algorithm [?, size, size, 12]
        ...
        surfaceArray = helpers.generateSurfaceArray(renderSize,
                                                    pixelsToAdd)  # Generate a grid Y,X between -1;1 to act as the pixel support of the rendering (computer the direction vector between each pixel and the light/view)

        # Do the renderings
        inputs = helpers.generateInputRenderings(rendererInstance, targetstoRender, self.batchSize, nbRenderings,
                                                 surfaceArray, renderingScene, jitterLightPos, jitterViewPos,
        ...
        if self.jitterRenderings:
            randomTopLeftCrop = tf.random_normal([self.batchSize, nbRenderings, 2], 0.0,
                                                 1.0)  # renderSize - self.cropSize, dtype=tf.int32)
            randomTopLeftCrop = randomTopLeftCrop * tf.exp(
                tf.random_normal([self.batchSize], 0.0, 1.0))  # renderSize - self.cropSize, dtype=tf.int32)
        ...
        inputs, targets = helpers.cutSidesOut(inputs, targetstoRender, randomTopLeftCrop, totalCropSize,
    ...
    def populateInNetworkFeedGraph(self, renderingScene, jitterLightPos, jitterViewPos, isTest, shuffle=True):
        ...
        dataset = dataset.map(self.__readMaterial, num_parallel_calls=int(
            multiprocessing.cpu_count() / 4))  # Divided by four as the cluster divides cpu availiability for each GPU
        ...
        if self.inputImageSize > renderCropSize:
            ...
            init_targets_batch = init_targets_batch[:, :, xyCropping[0]: xyCropping[0] + renderCropSize,
        ...
        # Do the renderings
        targets_batch, inputs_batch = self.__renderInputs(init_targets_batch, renderingScene, jitterLightPos,
                                                          jitterViewPos, self.mixMaterials, isTest, renderCropSize)
    ...
    def populateFeedGraph(self, shuffle=True):
        with tf.name_scope("load_images"):
            ...
            if inputRealSize > self.cropSize:
                ...
                inputs_batch = inputs_batch[:, :, xyCropping[0]: xyCropping[0] + self.cropSize,
                               xyCropping[0]: xyCropping[0] + self.cropSize, :]
                targets_batch = targets_batch[:, :, xyCropping[0]: xyCropping[0] + self.cropSize,
                                xyCropping[0]: xyCropping[0] + self.cropSize, :]
